...
### 6.0 Vercel 롤백 메커니즘 이해

**중요:** Vercel의 롤백 관련 작업은 3가지로 구분됩니다:

| 작업 | 설명 | 소스 코드 필요 | 제한 사항 |
|------|------|---------------|----------|
| **Promote** | 기존 배포를 Production으로 승격 | ❌ (기존 스냅샷 사용) | 플랜별로 접근 가능한 배포 수 다를 수 있음 |
| **Instant Rollback** | 직전 Production 배포로 복원 | ❌ (기존 스냅샷 사용) | 플랜별로 롤백 가능 범위 다를 수 있음 |
| **Redeploy** | 소스에서 새로 빌드 | ✅ (연결된 저장소 필요) | 저장소 변경 후 이전 코드 Redeploy 불가능할 수 있음 |

> **참고:** 배포는 불변(immutable) 스냅샷으로 프로젝트 레벨에 저장됩니다. Promote/Instant Rollback은 기존 스냅샷을 사용하므로 저장소 변경 후에도 **작동할 가능성이 있습니다**. 단, 플랜별 제한이나 정책 변경 가능성이 있으므로 실제 대시보드에서 확인이 필요합니다.

### 즉시 롤백 (1분 이내)

**방법:** Vercel 대시보드 → Deployments → 이전 배포 선택 → "Promote to Production" 또는 "Instant Rollback"

> ⚠️ **사전 확인 필요:** 마이그레이션 전 Vercel 대시보드에서 이전 배포들의 Promote/Rollback 옵션이 활성화되어 있는지 확인하세요.
...
| 방법 | 장점 | 단점 | 권장 |
|------|------|------|------|
| **A. 동일 Vercel 프로젝트 + 저장소 변경** | DNS 전파 없음, 도메인/GA 유지, 배포 히스토리 유지 | 설정 누락 시 장애 위험 | ✅ |
| **B. 코드 직접 교체 (dger-api 레포에 dger-map 복사)** | DNS 전파 없음, 빠른 전환 | 수동 작업 리스크, 히스토리/의미 혼재, 복사 실수 가능 | |
| **C. 새 Vercel 프로젝트 생성** | 기존 사이트 유지, 검증 유리 | 도메인 재연결 필요, GA 설정 변경 | |
| **D. 서브도메인 분리 (new.dger.kr)** | 병행 운영 가능 | 사용자 혼란, GA 분리 | |

### 8.1 저장소 변경 시 주의사항 (방법 A)

**환경 변수:**
- 프로젝트 레벨 환경 변수는 유지될 가능성이 높음
- 단, Vercel은 **Production / Preview / Development** 환경을 분리하여 관리
- 각 환경별로 다른 값이 설정되어 있을 수 있으므로 모든 환경 확인 필요
- `NEXT_PUBLIC_*` 변수는 빌드 시점에 포함되므로 **재빌드 후 확인** 필요

**롤백 가능성:**
- Promote/Instant Rollback: 기존 스냅샷 기반이므로 **작동할 가능성 있음**
- Redeploy: 저장소 변경 후 이전 코드 Redeploy는 **불가능할 가능성 있음**
- 실제 대시보드에서 사전 확인 권장

> ⚠️ **권장 사전 확인:** 마이그레이션 전 Vercel 대시보드에서 (1) 환경 변수 전체 목록 스크린샷, (2) 이전 배포의 Promote/Rollback 옵션 활성화 여부 확인

**트래픽/리스크 기준 권장 시나리오**
- 트래픽이 높고 다운타임 리스크가 큰 경우: **A** (DNS 전파 없음, 롤백 가능성 있음)
- 트래픽이 낮고 빠른 전환이 최우선인 경우: **B** (단, 수동 작업 리스크 수용 시)
- 장시간 사전 검증이 필요하거나 병행 운영이 필요한 경우: **C** 또는 **D**
...
| 2025-12-29 | 롤백 메커니즘 명확화 (Section 6.0): Promote/Instant Rollback/Redeploy 구분, 플랜별 제한 가능성 주의사항 추가 |
| 2025-12-29 | 저장소 변경 주의사항 추가 (Section 8.1): 환경 변수 분리, 롤백 가능성에 대한 신중한 표현으로 수정 |
